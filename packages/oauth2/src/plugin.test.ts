...
import { AuthError, createAuth, type AuthSessionRecord, type AuthUser } from '@ngriffin_uk/auth-core'
...
function setup(options: SetupOptions = {}) {
  ...
  const auth = createAuth<TestUser>({
    ...
    oauth2Auth({
      ...
      ...(options.clientAuthentication ? { clientAuthentication: options.clientAuthentication } : {}),
      ...(options.clientIdParameter ? { clientIdParameter: options.clientIdParameter } : {}),
      ...(options.pkce === undefined ? {} : { pkce: options.pkce }),
      ...(options.scopeSeparator ? { scopeSeparator: options.scopeSeparator } : {}),
      ...(options.tokenParameters ? { tokenParameters: options.tokenParameters } : {}),
      ...(options.tokenResponsePath ? { tokenResponsePath: options.tokenResponsePath } : {}),
      stateStore,
      fetch: request,
...
describe('oauth2Auth', () => {
  it('persists hashed state, adds PKCE, and completes a session', async () => {
    ...
    assert.deepEqual([...setupResult.states.values()][0]?.context, {
      intent: 'link-account'
    })

    const result = await setupResult.auth.providers.github.completeAuthorization({
  ...
  it('uses form encoding for HTTP Basic client credentials', async () => {
    ...
    assert.equal(
      new Headers(setupResult.getTokenRequestInit()?.headers).get('Authorization'),
  ...
  it('requires OIDC claims and azp for multiple audiences', async () => {
    ...
    const validUrl = await setupResult.auth.providers.github.startAuthorization()
