import type { AuthFlowResult, AuthUser, ExternalIdentity } from '@ngriffin_uk/auth-core'
import type { JwtAlgorithm, JwtClaims, JwtKeyResolver } from '@ngriffin_uk/auth-jwt'

export type ClientAuthentication = 'basic' | 'body' | 'none'
export type ClientSecretProvider = () => string | Promise<string>
export type OAuthTokenGrant = 'authorization_code' | 'refresh_token' | 'revoke'
...
export interface OAuthProviderConfig<Name extends string, User extends AuthUser> {
  readonly name: Name
...
export interface OAuthOperations<User extends AuthUser> {
  ...
  completeAuthorization(input: { readonly code: string; readonly state: string }): Promise<AuthFlowResult<User>>
