...
export interface OAuthProviderDefinition<Name extends string> {
  ...
  readonly tokenParameters?: Partial<Readonly<Record<OAuthTokenGrant, Readonly<Record<string, string>>>>>
...
export function defineOAuthProvider<const Name extends string>(
  ...
  return Object.freeze({
    ...definition,
    ...(definition.defaultScopes ? { defaultScopes: Object.freeze([...definition.defaultScopes]) } : {}),
...
export function createOAuthProvider<const Name extends string, User extends AuthUser>(
  definition: OAuthProviderDefinition<Name>,
  ...
  const config: OAuthProviderConfig<Name, User> = {
    ...
    ...(definition.revocationEndpoint ? { revocationEndpoint: definition.revocationEndpoint } : {}),
    ...(options.scopes ?? definition.defaultScopes ? { scopes: options.scopes ?? definition.defaultScopes } : {}),
    ...(options.clientAuthentication ?? definition.clientAuthentication
      ? {
          clientAuthentication: options.clientAuthentication ?? definition.clientAuthentication
        }
      : {}),
    pkce: definition.pkce,
    ...(definition.clientIdParameter ? { clientIdParameter: definition.clientIdParameter } : {}),
    ...(definition.scopeSeparator ? { scopeSeparator: definition.scopeSeparator } : {}),
    ...(definition.tokenParameters ? { tokenParameters: definition.tokenParameters } : {}),
    ...(definition.tokenResponsePath ? { tokenResponsePath: definition.tokenResponsePath } : {}),
    ...(definition.tokenHeaders ? { tokenHeaders: definition.tokenHeaders } : {}),
    ...mergeParameters(definition.authorizationParameters, options.authorizationParameters),
...
export function validateOAuthProviderDefinition(definition: OAuthProviderDefinition<string>): void {
  if (!PROVIDER_NAME_PATTERN.test(definition.name)) {
    throw new TypeError('OAuth provider name must be a lowercase slug.')
  }
  for (const endpoint of [definition.authorizationEndpoint, definition.tokenEndpoint, definition.revocationEndpoint]) {
  ...
  if (scopes.some((scope) => !scope || /\s/u.test(scope)) || new Set(scopes).size !== scopes.length) {
