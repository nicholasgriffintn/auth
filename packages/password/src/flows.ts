import { AuthError, type AuthFlowResult, type AuthPluginContext, type AuthUser } from '@ngriffin_uk/auth-core'

import type { PasswordInput, PasswordPluginConfig, PasswordPolicy } from './types.js'
import { validateCredentialInput, validatePasswordInput, normaliseEmailInput } from './validation.js'
...
export async function verifyEmail<User extends AuthUser>(
  ...
  const challenge = await runtime.context.consumeChallenge(token, 'password', ['email_verification'])
...
export async function requestPasswordReset<User extends AuthUser>(
  ...
  const issued = await runtime.context.issueChallenge('password', 'password_reset', { userId: account.user.id })
...
export async function resetPassword<User extends AuthUser>(
  ...
  validatePasswordInput({ email: 'password-reset@example.invalid', password: input.newPassword }, runtime.policy)
  const challenge = await runtime.context.consumeChallenge(input.token, 'password', ['password_reset'])
  const userId = payloadString(challenge.payload, 'userId')
...
export async function changePassword<User extends AuthUser>(
  ...
  if (!account || !(await runtime.config.hasher.verify(input.currentPassword, account.passwordHash))) {
    throw new AuthError('invalid_credentials')
  }
  validatePasswordInput({ email: account.user.email, password: input.newPassword }, runtime.policy)
  const passwordHash = await runtime.config.hasher.hash(input.newPassword)
...
async function issueEmailVerification<User extends AuthUser>(
  ...
  const issued = await runtime.context.issueChallenge('password', 'email_verification', { userId: user.id })
  await capability.send({
    user,
    token: issued.token,
    expiresAt: issued.expiresAt
...
async function verifiedPasswordAccount<User extends AuthUser>(
  ...
  const hasher = runtime.config.hasher
  if (isUpgradeableHasher(hasher)) {
    const verification = await hasher.verifyAndCheck(password, account.passwordHash)
    if (!verification.valid) {
      throw new AuthError('invalid_credentials')
    }
    if (verification.needsRehash) {
      await runtime.config.store.updatePassword(account.user.id, await hasher.hash(password))
    }
  } else if (!(await hasher.verify(password, account.passwordHash))) {
    throw new AuthError('invalid_credentials')
  }
  return account
}

function isUpgradeableHasher(
  hasher: PasswordRuntime<AuthUser>['config']['hasher']
): hasher is PasswordRuntime<AuthUser>['config']['hasher'] & {
  verifyAndCheck(
    password: string,
    passwordHash: string
  ): Promise<{ readonly valid: boolean; readonly needsRehash: boolean }>
} {
  return 'verifyAndCheck' in hasher && typeof hasher.verifyAndCheck === 'function'
}
