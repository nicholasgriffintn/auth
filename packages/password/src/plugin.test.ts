...
import { passwordAuth, type PasswordAccount, type PasswordPolicy } from './index.js'
...
function setup(
  ...
  const hasher = {
    ...
    ...(capabilities.upgradeHash
      ? {
          async verifyAndCheck(password: string, hash: string) {
            return {
              valid: hash === `outdated:${password}` || hash === `hashed:${password}`,
              needsRehash: hash === `outdated:${password}`
            }
          }
        }
      : {})
  ...
  const challenges = new Map<string, import('@ngriffin_uk/auth-core').AuthChallengeRecord>()
  ...
  const store = {
    ...
    async create(input: { email: string; passwordHash: string; emailVerified: boolean }) {
...
describe('passwordAuth', () => {
  ...
  it('returns one safe error for missing users and bad passwords', async () => {
    ...
    for (const attempt of attempts) {
      await assert.rejects(attempt, (error) => error instanceof AuthError && error.code === 'invalid_credentials')
    }
  ...
  it('verifies credentials without issuing a session', async () => {
    ...
    assert.equal(await setupResult.auth.validateSession('not-a-session'), null)
  })

  it('upgrades an outdated password hash after successful authentication', async () => {
    const user: TestUser = {
      id: 'user-1',
      email: 'person@example.com',
      createdAt: new Date('2026-01-01T00:00:00.000Z'),
    ...
    const setupResult = setup(
      {
        user,
        passwordHash: 'outdated:correct password',
        emailVerified: true
      },
      { upgradeHash: true }
    )
  ...
  it('uses opaque challenges for email verification', async () => {
    ...
    const signUpResult = await setupResult.auth.providers.password.signUp({
      email: 'person@example.com',
      password: 'long enough password'
    })
    ...
    const verified = await setupResult.auth.providers.password.verifyEmail({
      token: delivery.token
    })
  ...
  it('resets passwords without exposing whether an email exists', async () => {
    ...
    await setupResult.auth.providers.password.requestPasswordReset('missing@example.com')
    assert.equal(setupResult.deliveries.length, 1)
  })
})
