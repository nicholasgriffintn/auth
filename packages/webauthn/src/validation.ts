...
import { cborBytes, cborMap, cborNumber, cborString, decodeCbor, decodeFirstCbor, type CborValue } from './cbor.js'
...
export async function validateRegistration(
  ...
  const clientData = await validateClientData(response.clientDataJSON, 'webauthn.create', options)
  const attestation = cborMap(
    decodeCbor(decodeBoundedBase64Url(response.attestationObject, MAX_ATTESTATION_BYTES, 'attestation object')),
    'attestationObject'
  )
  ...
  if (
    ...
    !constantTimeEqual(
      parsed.credentialId,
      decodeBoundedBase64Url(response.credentialId, MAX_CREDENTIAL_ID_BYTES, 'credential ID')
    )
  ) {
  ...
  if (format === 'none') {
    ...
    if (!(await verifySignature(parsed.publicKeyOrThrow(), parsed.algorithm, signature, data))) {
...
export async function validateAuthentication(
  ...
  const clientData = await validateClientData(response.clientDataJSON, 'webauthn.get', options)
  ...
  const key = await importCredentialKey(options.publicKeyJwk, options.algorithm)
  const valid = await verifySignature(
    key,
    options.algorithm,
    decodeBoundedBase64Url(response.signature, MAX_SIGNATURE_BYTES, 'signature'),
...
async function validateClientData(
  ...
  const bytes = decodeBoundedBase64Url(encoded, MAX_CLIENT_DATA_BYTES, 'client data')
  let parsed: unknown
  try {
  ...
  const actualChallenge = decodeBoundedBase64Url(parsed['challenge'], 1024, 'challenge')
  if (!constantTimeEqual(actualChallenge, textEncoder.encode(options.challenge))) {
    throw new TypeError('WebAuthn challenge does not match.')
  }
  if (typeof parsed['origin'] !== 'string' || !options.origins.includes(parsed['origin'])) {
...
async function parseAuthenticatorData(
  ...
  if (registration) {
    ...
    const credentialIdLength = new DataView(bytes.buffer, bytes.byteOffset + 53, 2).getUint16(0)
...
function coseKeyToJwk(cose: ReadonlyMap<unknown, CborValue>): {
  ...
  if (keyType === 3 && algorithm === -257) {
    ...
    let firstNonZero = 0
    while (firstNonZero < encodedModulus.length && encodedModulus[firstNonZero] === 0) {
      firstNonZero += 1
    }
...
async function importCredentialKey(jwk: JsonWebKey, algorithm: WebAuthnAlgorithm): Promise<CryptoKey> {
...
async function verifySignature(
  ...
  const normalised = algorithm === 'ES256' ? ecdsaDerToRaw(signature, 32) : signature
...
function decodeBoundedBase64Url(value: string, maxBytes: number, field: string): Uint8Array {
  if (typeof value !== 'string' || value.length === 0 || value.length > Math.ceil((maxBytes * 4) / 3) + 2) {
