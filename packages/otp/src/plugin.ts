...
import { decodeBase64Url, encodeBase32, encodeBase64Url } from '@ngriffin_uk/auth-encoding'

import { createRecoveryCodes, hashRecoveryCode, validateRecoveryCodeCount } from './recovery-code.js'
import { verifyTotp } from './totp.js'
import type { OtpOperations, OtpPluginConfig } from './types.js'
import { createTotpUri } from './uri.js'
import { validateOtpParameters } from './validation.js'

export function otpAuth<User extends AuthUser>(config: OtpPluginConfig): AuthPlugin<'otp', OtpOperations<User>, User> {
  ...
  return {
    ...
    install(context) {
      return {
        ...
        createChallenge: (userId) => createChallenge(context, config, userId),
        verifyChallenge: (input) => verifyChallenge(context, config, input),
        useRecoveryCode: (input) => useRecoveryCode(context, config, input),
        verifyRecoveryCode: (input) => verifyRecoveryCode(context, config, input)
      }
    }
  }
}
...
async function startSetup<User extends AuthUser>(
  ...
  if (typeof input.accountName !== 'string' || !input.accountName.trim() || input.accountName.length > 320) {
...
async function verifySetup<User extends AuthUser>(
  ...
  const challenge = await context.readChallenge(input.token, 'otp', ['mfa_setup'])
  ...
  const verification = await verifyTotp(input.code, secret, context.now(), config.options)
  ...
  await context.consumeChallenge(input.token, 'otp', ['mfa_setup'])
  await config.store.saveCredential({
    userId,
    secret,
    lastAcceptedStep: verification.step,
    recoveryCodeHashes: payloadStrings(challenge.payload, 'recoveryCodeHashes')
  })
...
async function createChallenge<User extends AuthUser>(
  ...
  const [user, credential] = await Promise.all([context.users.findById(userId), config.store.findCredential(userId)])
  if (!user || !credential) throw new AuthError('invalid_input')
  const issued = await context.issueChallenge('otp', 'software_token_mfa', { userId })
  return {
...
async function verifyChallenge<User extends AuthUser>(
  ...
  const userId = await readUserChallenge(context, input.token)
  const credential = await config.store.findCredential(userId)
  if (!credential) throw new AuthError('invalid_credentials')
  const verification = await verifyTotp(input.code, credential.secret, context.now(), {
    ...config.options,
    ...(credential.lastAcceptedStep === undefined ? {} : { afterStep: credential.lastAcceptedStep })
  })
  if (
    !verification.valid ||
  ...
  await context.consumeChallenge(input.token, 'otp', ['software_token_mfa'])
  return authenticated(context, userId)
}
...
async function useRecoveryCode<User extends AuthUser>(
  ...
  const user = await verifyRecoveryCode(context, config, input)
  return authenticated(context, user.id)
}

async function verifyRecoveryCode<User extends AuthUser>(
  context: AuthPluginContext<User>,
  config: OtpPluginConfig,
  input: { readonly token: string; readonly code: string }
): Promise<User> {
  ...
  const consumed = codeHash && (await config.store.consumeRecoveryCode(userId, codeHash))
  if (!consumed) {
    await context.recordChallengeFailure(input.token)
...
async function readUserChallenge<User extends AuthUser>(
  context: AuthPluginContext<User>,
  token: string
): Promise<string> {
  const challenge = await context.readChallenge(token, 'otp', ['software_token_mfa'])
...
function validateConfig(config: OtpPluginConfig): void {
  if (typeof config.issuer !== 'string' || !config.issuer.trim() || config.issuer.length > 128) {
    throw new TypeError('OTP issuer is invalid.')
  }
  validateRecoveryCodeCount(config.recoveryCodeCount ?? 10)
  const options = config.options ?? {}
  validateOtpParameters(new Uint8Array(16), options.digits ?? 6, options.algorithm ?? 'SHA-1')
  if (
    options.periodSeconds !== undefined &&
    (!Number.isSafeInteger(options.periodSeconds) || options.periodSeconds < 1)
  ) {
    throw new TypeError('TOTP period must be a positive integer.')
  }
}

function payloadStrings(payload: Readonly<Record<string, unknown>>, field: string): readonly string[] {
  const value = payload[field]
