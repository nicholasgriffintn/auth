...
describe('PBKDF2 password hashing', () => {
  it('salts hashes, verifies passwords, and reports upgrades', async () => {
    ...
    assert.equal(await fastForTest.verify('correct horse battery staple', hash), true)
    assert.equal(await fastForTest.verify('wrong password', hash), false)

    const upgraded = createPbkdf2Hasher({ iterations: 2_000 })
    assert.deepEqual(await upgraded.verifyAndCheck('correct horse battery staple', hash), {
      valid: true,
      needsRehash: true
    })
  })
  ...
  it('rejects malformed hashes without throwing', async () => {
    ...
    assert.equal(await hasher.verify('password', `$pbkdf2-sha256$i=2000001$${'a'.repeat(16)}$${'b'.repeat(32)}`), false)
  })
})
