...
const HASH_PATTERN =
  /^\$pbkdf2-sha256\$i=(\d+)\$([A-Za-z0-9_-]+)\$([A-Za-z0-9_-]+)$/u;
...
export function createPbkdf2Hasher(
  ...
  const saltBytes = positiveInteger(
    options.saltBytes ?? DEFAULT_SALT_BYTES,
    "PBKDF2 salt bytes"
  );
  const keyBytes = positiveInteger(
    options.keyBytes ?? DEFAULT_KEY_BYTES,
    "PBKDF2 key bytes"
  );
  if (
    iterations > MAX_ITERATIONS ||
    saltBytes > MAX_SALT_BYTES ||
    keyBytes < 16 ||
