...
async function signUp<User extends AuthUser>(
  ...
  try {
    value = await runtime.client.request("SignUp", {
      ...
      UserAttributes: Object.entries(input.attributes ?? {}).map(
...
async function confirmSignUp<User extends AuthUser>(
  ...
  validateCode(code);
  const challenge = await runtime.context.consumeChallenge(token, PROVIDER, [
    "email_verification",
  ]);
  ...
  try {
    await runtime.client.request("ConfirmSignUp", {
      ...
      ...withOptional(
        "SecretHash",
        await runtime.client.secretHash(username)
      ),
...
async function resendConfirmationCode<User extends AuthUser>(
  ...
  try {
    ...
    if (
      cause instanceof CognitoServiceError &&
      cause.type === "UserNotFoundException"
    ) {
      return;
    }
    throw mapServiceError(cause);
...
async function startSignIn<User extends AuthUser>(
  ...
  return initiateAuth(runtime, input.username, "USER_AUTH", {
    USERNAME: input.username,
    ...(input.preferredChallenge
      ? { PREFERRED_CHALLENGE: input.preferredChallenge }
      : {}),
  });
}
...
async function initiateAuth<User extends AuthUser>(
  ...
  try {
    ...
    if (
      cause instanceof CognitoServiceError &&
      cause.type === "UserNotConfirmedException"
    ) {
      return issueEmailVerification(runtime, username, {});
    }
    if (
      cause instanceof CognitoServiceError &&
      cause.type === "PasswordResetRequiredException"
    ) {
...
async function respondToNewPassword<User extends AuthUser>(
  ...
  return respond(runtime, input.token, (challenge) => {
    ...
    return {
      ...
      ...Object.fromEntries(
        Object.entries(input.attributes ?? {}).map(([name, value]) => [
          `userAttributes.${name}`,
          value,
        ])
      ),
...
async function selectChallenge<User extends AuthUser>(
  ...
  return respond(runtime, input.token, (challenge) => {
    if (
      !["SELECT_CHALLENGE", "SELECT_MFA_TYPE"].includes(
...
async function respond<User extends AuthUser>(
  ...
  const record = await runtime.context.consumeChallenge(token, PROVIDER);
...
async function verifyMfaSetup<User extends AuthUser>(
  ...
  try {
    ...
    const session =
      optionalString(verified, "Session") ?? challenge.session;
    const secretHash = await runtime.client.secretHash(challenge.username);
    const value = await runtime.client.request("RespondToAuthChallenge", {
...
async function confirmForgotPassword<User extends AuthUser>(
  ...
  validatePassword(input.newPassword);
  const record = await runtime.context.consumeChallenge(
    input.token,
    PROVIDER,
    ["password_reset"]
  );
...
async function signOut<User extends AuthUser>(
  runtime: DirectRuntime<User>,
  accessToken: string
): Promise<void> {
...
async function authenticate<User extends AuthUser>(
  ...
  if (!runtime.context.identities) {
    throw new AuthError(
      ...
      "An identity store is required for Amazon Cognito authentication."
    );
  }
...
function parseAuthResponse(
  ...
  const authentication = optionalAuthenticationResult(
...
function publicParameters(
  ...
  const entries = Object.entries(response.ChallengeParameters ?? {}).filter(
    ([key]) => isPublicChallengeParameter(challengeName, key)
  );
  if (
    entries.length > MAX_PARAMETER_COUNT ||
    entries.some(
      ([key, value]) =>
        key.length > 256 || value.length > MAX_PARAMETER_LENGTH
    )
  ) {
    throw new AuthError("provider_error");
  }
...
function isPublicChallengeParameter(
  challengeName: string,
  key: string
): boolean {
  switch (challengeName) {
